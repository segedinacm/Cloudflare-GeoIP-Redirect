...
(function ( $ ) {
    ...
    $.fn.updateAdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        var _oRepeatableButtons = $( nodeThis ).find( '.wicfr-repeatable-field-buttons' )
            .filter( function() {                                       
                return $( this ).closest( '.wicfr-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
            });       
...
// @todo event.preventDefault();        
        ...
        var _sFieldID           = _oRepeatableAddButtons.first().closest( '.wicfr-field' ).attr( 'id' );
        var _nCurrentFieldCount = $( '#' + _sFieldsContainerID ).find( '.wicfr-field' )
            .filter( function() {                 
                return $( this ).closest( '.wicfr-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
            })        
            .length;
    ...
    $.fn.addAdminPageFrameworkRepeatableField = function( sFieldContainerID ) {
        ...
        if ( 'undefined' === typeof sFieldContainerID ) {
            var sFieldContainerID = $( this ).closest( '.wicfr-field' ).attr( 'id' );
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.wicfr-fields' );
        ...
        if ( ! _aOptions ) {     
            var _nodeButtonContainer = nodeFieldContainer.find( '.wicfr-repeatable-field-buttons' )
                .filter( function() {                 
                    return $( this ).closest( '.wicfr-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                })
                .first();
        ...
        var _oInnerFields       = nodeFieldsContainer.find( '.wicfr-field' ) 
                                    .filter( function() {                 
                                        return $( this ).closest( '.wicfr-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                                    });
        var _oRepeatableButtons = nodeFieldContainer.find( '.wicfr-repeatable-field-buttons' )
                                    .filter( function() {                 
                                        return $( this ).closest( '.wicfr-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                                    }); 
        ...
        $( nodeNewField ).find( '.wicfr-field' ).addBack().trigger( 
            'wicfr_repeated_field', 
            [ 
                0, // call type, 0 : repeatable fields, 1: repeatable sections
                jQuery( nodeNewField ).closest( '.wicfr-fields' )    // model container
            ]
        );
        ...
        var nodeRemoveButtons = nodeFieldsContainer
            ...
            .filter( function() {                      
                return $( this ).closest( '.wicfr-fields' ).attr( 'id' ) === _sFieldsContainerID;                
            })
        ...
        nodeFieldsContainer.children( '.wicfr-field' ).children( '.delimiter' ).show().last().hide();
        ...
        var _incrementFieldAttributes = function( oElement, oFieldsContainer ) {
            ...
            oElement.find( '.wicfr-field,.wicfr-fields,.wicfr-fieldset' ).incrementAttributes(
    ...
    $.fn.removeAdminPageFrameworkRepeatableField = function() {
        ...
        var nodeFieldContainer  = $( this ).closest( '.wicfr-field' );
        var nodeFieldsContainer = $( this ).closest( '.wicfr-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );
        var _aOptions = nodeFieldsContainer.data( 'repeatable' );
