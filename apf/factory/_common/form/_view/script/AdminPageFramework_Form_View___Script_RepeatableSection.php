...
( function( $ ) {
    ...
    $.fn.updateAdminPageFrameworkRepeatableSections = function( aSettings ) {
        ...
        var _oThis                = this;
        var _sSectionsContainerID = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.wicfr-sections' ).attr( 'id' );
        ...
        var _sSectionID           = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.wicfr-section' ).attr( 'id' );
        var _nCurrentSectionCount = jQuery( '#' + _sSectionsContainerID ).find( '.wicfr-section' ).length;
    ...
    $.fn.addAdminPageFrameworkRepeatableSection = function( sSectionContainerID ) {
        ...
        if ( 'undefined' === typeof sSectionContainerID ) {
            var sSectionContainerID = $( this ).closest( '.wicfr-section' ).attr( 'id' );
        }
        var nodeSectionContainer    = $( '#' + sSectionContainerID );
        var nodeNewSection          = nodeSectionContainer.clone(); // clone without bind events.
        var nodeSectionsContainer   = nodeSectionContainer.closest( '.wicfr-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );        
        var nodeTabsContainer       = $( this ).closest( '.wicfr-section-tabs-contents' )
            .children( '.wicfr-section-tabs' )
            .first();
        ...
        if ( _sMaxNumberOfSections != 0 && nodeSectionsContainer.find( '.wicfr-section' ).length >= _sMaxNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.wicfr-repeatable-section-buttons' ).last();
        ...
        var _sSectionTabSlug = nodeNewSection.find( '.wicfr-section-caption' ).first().attr( 'data-section_tab' );
        if ( ! _sSectionTabSlug || _sSectionTabSlug === '_default' ) {
            nodeNewSection.find( '.wicfr-section-title' ).not( '.wicfr-collapsible-section-title' ).hide();
        }
        ...
        if( 'function' === typeof nodeNewSection.enableAdminPageFrameworkCollapsibleButton ){ 
            nodeNewSection.find( '.wicfr-collapsible-sections-title, .wicfr-collapsible-section-title' ).enableAdminPageFrameworkCollapsibleButton();
        }
        ...
        $( nodeNewSection ).find( '.wicfr-field' ).each( function( iFieldIndex ) {
            ...
            // 3.8.8 
            $( this ).trigger( 
                'wicfr_repeated_field', 
                [
                    1, // call type, 0: repeatable fields, 1: repeatable sections, 
                    jQuery( nodeNewSection ).closest( '.wicfr-sections' )    // model container
                ]
            );              
        ...
        nodeNewSection.find( '.wicfr-fields.sortable' ).each( function() {
        ...
        if ( nodeTabsContainer.length > 0 && ! nodeSectionContainer.hasClass( 'is_subsection_collapsible' ) ) {
            ...
            nodeTabsContainer.closest( '.wicfr-section-tabs-contents' ).createTabs( 'refresh' );
        ...
        var _incrementAttributes = function( oElement, iSectionsCount, oSectionsContainer ) {
            ...
            $( oElement ).find( 'tr.wicfr-fieldrow, .wicfr-fieldset, .wicfr-fields, .wicfr-field, table.form-table, input,textarea,select,option' )
                .incrementAttribute( 
                    'id', 
                    iSectionsCount,
                    _sSectionIDModel
            ...
            $( oElement ).find( '.wicfr-fields' ).incrementAttribute( 
                'data-field_tag_id_model',
                iSectionsCount,
                _sSectionIDModel
            );
            $( oElement ).find( '.wicfr-fields' ).incrementAttributes( 
        ...
        // @todo this may be able to be removed
            $( oElement ).find( '.wicfr-fieldset' ).incrementAttribute( 
                'data-field_id',
                iSectionsCount,
                _sSectionIDModel
            );
    ...
    $.fn.removeAdminPageFrameworkRepeatableSection = function() {
        ...
        var nodeSectionContainer    = $( this ).closest( '.wicfr-section' );
        var sSectionContainerID     = nodeSectionContainer.attr( 'id' );
        var nodeSectionsContainer   = $( this ).closest( '.wicfr-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );        
        var nodeTabsContainer       = $( this ).closest( '.wicfr-section-tabs-contents' )
            .children( '.wicfr-section-tabs' )
            .first();
        var nodeTabs                = nodeTabsContainer.children( '.wicfr-section-tab' );
        ...
        if ( _sMinNumberOfSections != 0 && nodeSectionsContainer.find( '.wicfr-section' ).length <= _sMinNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.wicfr-repeatable-section-buttons' ).last();
        ...
        nodeSectionContainer.fadeOut( _iFadeout, function() { 
            ...
            if ( 1 === _nodeRemoveButtons.length ) {
                ...
                var _sSectionTabSlug = nodeSectionsContainer.find( '.wicfr-section-caption' ).first().attr( 'data-section_tab' );
                if ( ! _sSectionTabSlug || '_default' === _sSectionTabSlug ) {
                    nodeSectionsContainer.find( '.wicfr-section-title' ).first().show();
                }                
            }            
...
var _iSectionIndex = _iIterationIndex;
            ...
            // Call the registered callback functions.
            // @deprecated  3.6.0
            // $( this ).find( '.wicfr-field' ).each( function( iFieldIndex ) {    
...
jQuery( document ).ready( function() {
    ...
    // Adds the buttons
    jQuery( '#{$sContainerTagID} .wicfr-section-caption' ).each( function(){
        
        jQuery( this ).show();
        
        var _oButtons = jQuery( $_sButtonsHTML );
        if ( jQuery( this ).children( '.wicfr-collapsible-section-title' ).children( 'fieldset' ).length > 0 ) {
            _oButtons.addClass( 'section_title_field_sibling' );
        }
        var _oCollapsibleSectionTitle = jQuery( this ).find( '.wicfr-collapsible-section-title' );
...
JAVASCRIPTS;
        return "<script type='text/javascript' class='wicfr-section-repeatable-script'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
    static private function ___getContainerAttributes(array $aArguments, $oMsg) {
        $_aAttriubtes = array('class' => self::getClassAttribute('wicfr-repeatable-section-buttons', empty($aArguments['disabled']) ? '' : 'disabled'),);
