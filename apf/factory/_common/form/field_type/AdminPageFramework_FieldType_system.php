...
class AdminPageFramework_FieldType_system extends AdminPageFramework_FieldType {
    ...
    protected function getStyles() {
        return ".wicfr-field-system {width: 100%;}.wicfr-field-system .wicfr-input-label-container {width: 100%;}.wicfr-field-system textarea {background-color: #f9f9f9; width: 97%; outline: 0; font-family: Consolas, Monaco, monospace;white-space: pre;word-wrap: normal;overflow-x: scroll;}";
    }
    protected function getField($aField) {
        $_aInputAttributes = $aField['attributes'];
        $_aInputAttributes['class'].= ' system';
        unset($_aInputAttributes['value']);
        return $aField['before_label'] . "<div class='wicfr-input-label-container'>" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . ($aField['label'] && !$aField['repeatable'] ? "<span " . $this->getLabelContainerAttributes($aField, 'wicfr-input-label-string') . ">" . $aField['label'] . "</span>" : "") . "<textarea " . $this->getAttributes($_aInputAttributes) . " >" . esc_textarea($this->_getSystemInfomation($aField['value'], $aField['data'], $aField['print_type'])) . "</textarea>" . $aField['after_input'] . "</label>" . "</div>" . $aField['after_label'];
    }
    ...
    private function _getClientInfo($bGenerateInfo = true) {
        ...
        return empty($_aBrowser) ? __('No browser information found.', 'wicfr') : $_aBrowser;
    }
    ...
    private function _getSiteInfo() {
        global $wpdb;
        return array(__('Version', 'wicfr') => $GLOBALS['wp_version'], __('Language', 'wicfr') => $this->getSiteLanguage(), __('Memory Limit', 'wicfr') => $this->getReadableBytes($this->getNumberOfReadableSize(WP_MEMORY_LIMIT)), __('Multi-site', 'wicfr') => $this->getAOrB(is_multisite(), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Permalink Structure', 'wicfr') => get_option('permalink_structure'), __('Active Theme', 'wicfr') => $this->_getActiveThemeName(), __('Registered Post Statuses', 'wicfr') => implode(', ', get_post_stati()), 'WP_DEBUG' => $this->getAOrB($this->isDebugMode(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), 'WP_DEBUG_LOG' => $this->getAOrB($this->isDebugLogEnabled(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), 'WP_DEBUG_DISPLAY' => $this->getAOrB($this->isDebugDisplayEnabled(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('Table Prefix', 'wicfr') => $wpdb->prefix, __('Table Prefix Length', 'wicfr') => strlen($wpdb->prefix), __('Table Prefix Status', 'wicfr') => $this->getAOrB(strlen($wpdb->prefix) > 16, $this->oMsg->get('too_long'), $this->oMsg->get('acceptable')), 'wp_remote_post()' => $this->_getWPRemotePostStatus(), 'wp_remote_get()' => $this->_getWPRemoteGetStatus(), __('Multibite String Extension', 'wicfr') => $this->getAOrB(function_exists('mb_detect_encoding'), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('WP_CONTENT_DIR Writeable', 'wicfr') => $this->getAOrB(is_writable(WP_CONTENT_DIR), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Active Plugins', 'wicfr') => PHP_EOL . $this->_getActivePlugins(), __('Network Active Plugins', 'wicfr') => PHP_EOL . $this->_getNetworkActivePlugins(), __('Constants', 'wicfr') => $this->_getDefinedConstants('user'),);
    }
    ...
    private function _getPHPInfo($bGenerateInfo = true) {
        ...
        self::$_aPHPInfo = array(__('Version', 'wicfr') => phpversion(), __('Safe Mode', 'wicfr') => $this->getAOrB(@ini_get('safe_mode'), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Memory Limit', 'wicfr') => @ini_get('memory_limit'), __('Upload Max Size', 'wicfr') => @ini_get('upload_max_filesize'), __('Post Max Size', 'wicfr') => @ini_get('post_max_size'), __('Upload Max File Size', 'wicfr') => @ini_get('upload_max_filesize'), __('Max Execution Time', 'wicfr') => @ini_get('max_execution_time'), __('Max Input Vars', 'wicfr') => @ini_get('max_input_vars'), __('Argument Separator', 'wicfr') => @ini_get('arg_separator.output'), __('Allow URL File Open', 'wicfr') => $this->getAOrB(@ini_get('allow_url_fopen'), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Display Errors', 'wicfr') => $this->getAOrB(@ini_get('display_errors'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Log Errors', 'wicfr') => $this->getAOrB(@ini_get('log_errors'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Error log location', 'wicfr') => @ini_get('error_log'), __('Error Reporting Level', 'wicfr') => $_oErrorReporting->getErrorLevel(), __('FSOCKOPEN', 'wicfr') => $this->getAOrB(function_exists('fsockopen'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), __('cURL', 'wicfr') => $this->getAOrB(function_exists('curl_init'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), __('SOAP', 'wicfr') => $this->getAOrB(class_exists('SoapClient'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), __('SUHOSIN', 'wicfr') => $this->getAOrB(extension_loaded('suhosin'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), 'ini_set()' => $this->getAOrB(function_exists('ini_set'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')),) + $this->getPHPInfo() + array(__('Constants', 'wicfr') => $this->_getDefinedConstants(null, 'user'));
        return self::$_aPHPInfo;
    }
    private function _getWebServerInfo($bGenerateInfo = true) {
        return $bGenerateInfo ? array(__('Web Server', 'wicfr') => $_SERVER['SERVER_SOFTWARE'], 'SSL' => $this->getAOrB(is_ssl(), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Session', 'wicfr') => $this->getAOrB(isset($_SESSION), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('Session Name', 'wicfr') => esc_html(@ini_get('session.name')), __('Session Cookie Path', 'wicfr') => esc_html(@ini_get('session.cookie_path')), __('Session Save Path', 'wicfr') => esc_html(@ini_get('session.save_path')), __('Session Use Cookies', 'wicfr') => $this->getAOrB(@ini_get('session.use_cookies'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Session Use Only Cookies', 'wicfr') => $this->getAOrB(@ini_get('session.use_only_cookies'), $this->oMsg->get('on'), $this->oMsg->get('off')),) + $_SERVER : '';
    }
    }
